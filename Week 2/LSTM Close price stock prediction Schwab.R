...
# Data Wranging
scale_factors <- c(mean(CharlesSchwab$Close), sd(CharlesSchwab$Close))
scale_factors
...
x_test <- CharlesSchwab$Close[(nrow(scaled_train) - prediction + 1):nrow(scaled_train)]
...
input_ts <- timetk::tk_ts(CharlesSchwab$Close, 
                          start = c(2000, 12), 
                          end = c(2021,12 ), 
                          frequency = 12)
input_ts
