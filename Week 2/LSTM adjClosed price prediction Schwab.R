...
#Load in Data
CharlesSchwab = read.csv("/Users/korigray/Documents/GitHub/Final class project/Final-Project/Week 2/SCHW.csv")

head(CharlesSchwab)

# Data Wranging
scale_factors <- c(mean(CharlesSchwab$Adj.Close), sd(CharlesSchwab$Adj.Close))
scale_factors

scaled_train <- CharlesSchwab %>%
  dplyr::select(Adj.Close) %>%
  dplyr::mutate(Adj.Close = (Adj.Close - scale_factors[1]) / scale_factors[2])
...
x_test <- CharlesSchwab$Adj.Close[(nrow(scaled_train) - prediction + 1):nrow(scaled_train)]
...
input_ts <- timetk::tk_ts(CharlesSchwab$Adj.Close, 
                          start = c(2000, 12), 
                          end = c(2021,12 ), 
                          frequency = 12)
input_ts
