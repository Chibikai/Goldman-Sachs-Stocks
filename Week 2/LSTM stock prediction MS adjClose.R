...
#Load in Data
MorganStanley = read.csv("/Users/korigray/Documents/GitHub/Final class project/Final-Project/Week 2/MS.csv")

head(MorganStanley)

# Data Wranging
scale_factors <- c(mean(MorganStanley$Adj.Close), sd(MorganStanley$Adj.Close))
scale_factors

scaled_train <- MorganStanley %>%
...
x_test <- MorganStanley$Adj.Close[(nrow(scaled_train) - prediction + 1):nrow(scaled_train)]
...
input_ts <- timetk::tk_ts(MorganStanley$Adj.Close, 
                          start = c(2000, 12), 
                          end = c(2021,12 ), 
                          frequency = 12)
input_ts
